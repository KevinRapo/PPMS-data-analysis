...
folder_path = "/PdCorrection tables" #Laptop
...
def plot_MvsT(separated_MvsT, MvsT_indices, const_H_values):
    ...
    for i in range(len(MvsT_indices)):
        ...
        ax.grid(True)
        fig.savefig(f"MvsT_graph_at_{val}K.png",bbox_inches = "tight", dpi = 200)
...
def plot_MvsH(separated_MvsH, const_T_values, interpolated_MvsH):
    ...
    for i in range(len(separated_MvsH)):
        ...
        ax.grid(True)
        fig.savefig(f"MvsH_graph_at_{val}K.png",bbox_inches = "tight", dpi = 200)
        i += 1
...
dataset = unfiltered_MvsH_T_values[1] #see on discrete data

dataset = separated_MvsT[1][1]["Temperature (K)"]  #see on pidev data

smoothed_dataset = dataset.round(decimals=1)

#smoothed_dataset=np.array(['b', 'b', 'a', 'c', 'b'])

codes, uniques = pd.factorize(smoothed_dataset)

smoothed_dataset.size

uniques.size

ratio = uniques.size/smoothed_dataset.size
ratio
